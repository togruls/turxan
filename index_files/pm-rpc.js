...
/***/ function(module, exports, __webpack_require__) {
	...
	var _windowModule = __webpack_require__(4);

	var windowModule = _interopRequireWildcard(_windowModule);

	var _listeners = __webpack_require__(6);

	function _interopRequireWildcard(obj) { if (obj && obj.__esModule) { return obj; } else { var newObj = {}; if (obj != null) { for (var key in obj) { if (Object.prototype.hasOwnProperty.call(obj, key)) newObj[key] = obj[key]; } } newObj.default = obj; return newObj; } }

	var _apps = {};

	var handleSendAPI = function handleSendAPI(context, event) {
	    var argsToCall = [];
	    var args = event.data.args;
	    var call = event.data.call;
	    var appId = event.data.appId;
	    var intent = event.data.intent;
	    if (intent === 'invoke' && args && call) {
	        (function () {
	            var responsePromiseId = 0;
	            ...
	            if (fncToInvoke) {
	                if (responsePromiseId > 0) {
	                    Promise.all([fncToInvoke.apply(undefined, argsToCall)]).then(function (args) {
	                        if (context === 'frame') {
	                            targetUtils.getTarget().postMessage({
	                                intent: 'response-promise',
	                                promiseId: responsePromiseId,
	                                args: args
	                            }, "*");
	                        } else {
	...
	var set = function set(appId, app) {
	    ...
	    if (windowModule.isWorker()) {
	        ...
	        (0, _listeners.addListener)(windowModule.get(), 'message', handleSendAPI.bind(this, 'worker'), false);
	    }
	};
	...
	var sendTo = function sendTo(appId, target) {
	    ...
	    if (app) {
	        ...
	        (0, _listeners.addListener)(windowModule.get(), 'message', handleSendAPI.bind(this, 'frame'), false);
	    } else {
	        console.warn('No api to send, please call rpc.set w/ proper API');
	    }
	};
	...
	var get = function get() {
	    ...
	    if (windowModule.isWorker()) {
	        return new Promise(function (resolve) {
	            (0, _listeners.addListener)(windowModule.get(), 'message', handleGet.bind(undefined, resolve, options.appId, 'worker'), false);
	        });
	    } else {
	        targetUtils.setTargetId(options.targetId);
	        targetUtils.setInitiator(options.initiator);
	        return new Promise(function (resolve) {
	            (0, _listeners.addListener)(windowModule.get(), 'message', handleGet.bind(undefined, resolve, options.appId, 'frame'), false);
	        });
	    }
	};
...
/***/ function(module, exports, __webpack_require__) {
	...
	exports.getInitiator = exports.setInitiator = exports.setTargetId = exports.getTargetId = exports.setTarget = exports.getTarget = undefined;
	...
	var _initiator = void 0;

	var getTarget = function getTarget() {
	    if (_target && !_initiator) {
	        return _target;
	    } else if (windowModule.isWorker() && _targetId) {
	        return windowModule.getContentWindow(_targetId);
	    } else if (_initiator) {
	        return windowModule.getContentWindow(_initiator);
	    } else if (_targetId === 'parent') {
	        return parent;
	    }
	};
	...
	var setInitiator = function setInitiator(initiator) {
	...
	exports.setInitiator = setInitiator;
	exports.getInitiator = getInitiator;
...
/***/ function(module, exports) {
	...
	var getContentWindow = function getContentWindow(id) {
	...
	var buildAPI = function buildAPI(data, context) {
	    ...
	    fn.forEach(function (prop) {
	        api[prop] = function () {
	            ...
	            return new Promise(function (resolve) {
	                var argsToSend = [].concat(Array.prototype.slice.call(_arguments));
	                var promiseId = promiseUtils.getNextPromiseId();
	                argsToSend.push({ promiseId: promiseId });
	                promiseUtils.setPromiseById(promiseId, resolve);
	                if (context === 'frame') {
	                    targetUtils.getTarget().postMessage({
	                        appId: data.appId, call: prop, args: argsToSend, intent: 'invoke'
	                    }, '*');
	                } else {
	                    windowModule.get().postMessage({
	                        appId: data.appId, call: prop, args: argsToSend, intent: 'invoke'
	                    });
	                }
	            });
...
/***/ function(module, exports) {
	...
	var listeners = [];
	function addListener(element, event, handler, capture) {
	    removeAllListeners();
	    element.addEventListener(event, handler, capture);
	    listeners.push({ element: element, event: event, handler: handler, capture: capture });
	}
